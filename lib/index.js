        ...
        beanstalk_in
            .on('connect', function() {
                ...
                beanstalk_in.watch(acttopic, function(err, numwatched) {
                    if (err) {
                        return in_err('watch/' + acttopic, err);
                    }

                    function do_reserve() {
                        if (beanstalk_in.closed$) {
                            return;
                        }

                        beanstalk_in.reserve(function(err, jobid, payload) {
                            if (err) {
                                return in_err('reserve/' + jobid, err);
                            }
                                ...
                                tu.handle_request(seneca, data, listen_options, function(out) {
                                    ...
                                    beanstalk_out.put(
                                        ...
                                        outstr, function(err, outjobid) {
                                            if (err) {
                                                return out_err('put', err);
                                            }

                                            beanstalk_in.destroy(jobid, function(err) {
                                                if (err) {
                                                    return in_err('destroy/' + jobid, err);
                                                }
        ...
        beanstalk_in
            .on('connect', function() {
                ...
                beanstalk_in.watch(restopic, function(err, numwatched) {
                    if (err) {
                        return in_err('watch/' + restopic, err);
                    }
                            ...
                            if (data) {
                                ...
                                if (complete) {
                                    ...
                                    beanstalk_in.release(
                                        ...
                                        client_options.delay, function(err) {
                                            if (err) {
                                                return in_err('release/' + jobid, err);
                                            }
        ...
        beanstalk_out
            .on('connect', function() {
                ...
                beanstalk_out.use(acttopic, function(err, numwatched) {
                    if (err) {
                        return out_err('use/' + acttopic, err);
                    }
                    ...
                    client = function(args, done) {
                        ...
                        try {
                            beanstalk_out.put(
                                ...
                                outstr, function(err, outjobid) {
                                    var clear = function(jobid) {
                                        return function() {
                                            beanstalk_out.destroy(jobid, function(err) {
                                                beanstalk_out.touch(jobid, function(err) {
                                    ...
                                    setTimeout(clear(outjobid), 4000);
                                    if (err) {
                                        return out_err('put/' + outjobid, err);
                                    }
                                });
                        } catch ( e ) {
                            if (e) {
                                return out_err('put', e);
                            }
                        }
                    };
                    client.id$ = nid();
